...
parser.add_argument("--test_dir", type=str, default=r"./data\librimix\Libri2Mix\wav16k\min\metadata\test", help="Test directory including the csv files")
parser.add_argument("--task", type=str, default="enh_single", choices=["sep_clean", "sep_noisy","enh_single"])
parser.add_argument("--use_gpu", type=int, default=1, help="Whether to use the GPU for model execution")
parser.add_argument("--exp_dir", default="exp/half", help="Experiment root")
parser.add_argument("--out_dir", type=str, default="results/enh_single", help="Directory in exp_dir where the eval results will be stored")
...
def main(conf):
    ...
    if not os.path.exists(os.path.join(eval_save_dir, "final_metrics.json")):
        if ".pth" in conf["ckpt_path"] or ".bin" in conf["ckpt_path"]:
